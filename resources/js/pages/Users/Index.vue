...
const updateForms = reactive(
    Object.fromEntries(
        ...
        string,
        ReturnType<typeof useForm<{ role: string; status: string }>>
    >,
);
...
Object.entries(updateForms).forEach(([userId, form]) => {
    ...
    useFormErrorToast(
        ...
        managedUser
            ? `Не удалось обновить пользователя ${managedUser.email}`
    ...
    createForm.post(route('users.store'), {
        ...
        onSuccess: () => {
            ...
            toast.add({
                ...
                description:
                    'Теперь добавьте его в проект, чтобы он получил доступ к секретам.',
                actions: [
                    {
                ...
                <template #header>
                    ...
                    >
                        <div class="space-y-1">
                            ...
                            <p class="text-sm text-muted">
                                Здесь видно, кто вообще может пользоваться
                                Access Atlas и с каким уровнем доступа.
                            </p>
                        </div>
