...
import { criticalityLabel, projectRoleLabel, projectStatusLabel, secretStorageModeLabel, sensitivityLabel } from '@/lib/labels'
...
useFormErrorToast(computed(() => projectForm.errors), 'Не удалось обновить проект')
useFormErrorToast(computed(() => membershipForm.errors), 'Не удалось добавить пользователя в проект')
...
function addParticipants(): void {
  ...
  selectedIds.forEach((userId, index) => {
    router.post(route('projects.users.store', props.project.id), {
      user_id: userId,
...
function removeParticipant(userId: string): void {
  router.delete(route('projects.users.destroy', { project: props.project.id, user: userId }), {
    preserveScroll: true,
  })
}
      ...
      <UCard>
        <template #header>
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <UBadge variant="soft" color="neutral">
                  {{ criticalityLabel(props.project.criticality) }}
                </UBadge>
              ...
              <p class="text-sm text-muted">
                {{ props.project.description || 'Краткое описание проекта пока не заполнено.' }}
              </p>
            ...
            <UButton
              v-if="props.can_archive && !props.project.archived_at"
              color="error"
              variant="soft"
      ...
      <section class="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <UCard>
          <template #header>
            <div class="space-y-1">
              ...
              <p class="text-sm text-muted">
                Название, репозиторий и критичность можно менять здесь.
              </p>
            </div>
          </template>
          ...
          >
            ...
            >
              <UInput v-model="projectForm.repository_url" class="w-full" />
            </UFormField>
        ...
        <div class="space-y-4">
            ...
            >
              ...
              <div class="flex justify-end">
                <UButton
                  ...
                  :disabled="membershipForm.user_ids.length === 0"
                  icon="i-lucide-user-plus"
                  type="submit"
                >
                  {{ membershipForm.user_ids.length > 1 ? `Прикрепить ${membershipForm.user_ids.length} чел.` : 'Прикрепить к проекту' }}
                </UButton>
              </div>
            </UForm>
          </UCard>
          ...
          <UCard>
            ...
            <div class="space-y-3">
              <div
                v-for="environment in props.project.environments"
                :key="environment.id"
        ...
        <div class="space-y-3">
          ...
          >
            ...
            <div class="flex items-center gap-2">
              ...
              <UButton
                v-if="props.can_manage && membership.status !== 'left'"
                color="error"
                variant="ghost"
                size="xs"
        ...
        <template #header>
          <div class="flex flex-wrap items-center justify-between gap-4">
            <div class="space-y-1">
              ...
              <p class="text-sm text-muted">
                Все секреты, привязанные к этому проекту. Раскройте нужный и скопируйте.
              </p>
            </div>
          ...
          >
            <div class="flex items-start justify-between gap-3">
              <div class="space-y-1">
                ...
                <p class="text-sm text-muted">
                  {{ secret.environment_name }} · {{ secret.secret_type.replaceAll('_', ' ') }}
                </p>
              </div>
              ...
              <p class="text-xs text-muted">
                {{
                  secret.storage_mode === 'encrypted_value'
                    ? 'Зашифровано'
                    : secretStorageModeLabel(secret.storage_mode)
                }}
              </p>
