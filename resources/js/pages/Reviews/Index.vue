...
useFormErrorToast(
    ...
    'Не удалось создать задачу ревью',
);

const targetItems = computed(
    () => props.target_options[form.target_type] ?? [],
);
...
Ревью
                ...
                <p class="mt-2 max-w-2xl text-sm text-muted">
                    Очередь ротации, проверки и ownership review для
                    ...
                    >
                        <USelect
                            ...
                            :items="[
                                ...
                                {
                                    label: 'Проверка секрета',
                                    value: 'secret_verification',
                                },
                                {
                                    label: 'Ротация секрета',
                                    value: 'secret_rotation',
                                },
                                {
                                    label: 'Проверка ownership',
                                    value: 'ownership_review',
                                },
                            ]"
                        />
                    </UFormField>
...
Срок: {{ task.due_at }}
                    ...
                    <template #footer>
                        <UButton
                            v-if="
                                props.can_manage && task.status !== 'completed'
                            "
                            variant="soft"
                            color="success"
                            icon="i-lucide-check"
