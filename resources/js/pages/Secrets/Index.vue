...
import type { RevealedSecret, Secret, SecretsIndexPageProps } from '@/types/secrets'
import { Head, useForm, usePage } from '@inertiajs/vue3'
import { computed, ref, watch } from 'vue'
import { route } from 'ziggy-js'
import { useFormErrorToast } from '@/composables/useFormErrorToast'
import AppLayout from '@/layouts/AppLayout.vue'
import { projectStatusLabel, secretStorageModeLabel, sensitivityLabel } from '@/lib/labels'
...
const revealPolicyOptions = [
  { label: 'Владелец проекта или security admin', value: 'project_owner_or_security_admin' },
  { label: 'Владелец, техлид или менеджер проекта', value: 'project_manager_or_higher' },
...
useFormErrorToast(computed(() => form.errors), 'Не удалось сохранить секрет')

const revealForm = useForm({
  reason: '',
})

useFormErrorToast(computed(() => revealForm.errors), 'Не удалось раскрыть секрет')

const revealedSecret = computed<RevealedSecret | null>(() =>
  (page.props.flash?.revealed_secret as RevealedSecret | null | undefined) ?? null,
)
...
const currentStep = computed(() => {
  ...
  if (isEncryptedMode.value ? !form.encrypted_value : !form.external_reference) {
    return 2
  }
...
function createSecretType(value: string): void {
  ...
  if (secretTypeOptions.value.some(option => option.value === optionValue)) {
...
function createSecret(): void {
  form.transform(data => ({
    ...
    external_reference: isEncryptedMode.value ? null : data.external_reference || null,
    encrypted_value: isEncryptedMode.value ? data.encrypted_value || null : null,
...
async function copyReference(): Promise<void> {
  ...
  await copyText(revealedSecret.value.external_reference, 'Reference скопирован')
}
</script>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-1 flex-col gap-6 p-4">
          ...
          <template #header>
            <div class="space-y-3">
              ...
              <p class="text-sm text-muted">
                Сначала определяем контекст, потом способ хранения. Если вставляете пароль, token или key прямо сюда,
                значение будет зашифровано при сохранении.
              </p>
            </div>
          </template>
        ...
        <UCard variant="soft">
          <template #header>
            <div class="space-y-2">
              ...
              <p class="text-sm text-muted">
                {{ form.project_id ? 'Контекст уже выбран, осталось только корректно зафиксировать сам секрет.' : 'Сначала выберите проект и окружение, чтобы реестр понял контекст.' }}
              </p>
            </div>
          </template>
            ...
            <div class="grid grid-cols-2 gap-3">
              <UCard variant="outline">
                ...
                <p class="mt-2 text-sm font-medium">
                  {{ secretStorageModeLabel(form.storage_mode) }}
                </p>
              </UCard>
              <UCard variant="outline">
                <p class="text-xs font-medium uppercase tracking-[0.2em] text-muted">
                ...
                <p class="mt-2 text-sm font-medium">
                  {{ form.owner_user_id ? 'Назначен' : 'Не выбран' }}
                </p>
              </UCard>
            </div>
            ...
            <div class="grid gap-3 md:grid-cols-2">
              ...
              <UCard variant="outline">
                ...
                <ul class="mt-3 space-y-2 text-sm text-muted">
                  <li>Значение не показывается в листингах и не участвует в глобальном поиске.</li>
                  <li>По умолчанию подставляется текущий ответственный, чтобы форму можно было заполнить за один проход.</li>
                  <li>Сложные настройки спрятаны, чтобы не мешать основному действию.</li>
                </ul>
              </UCard>
            </div>
          </div>
          ...
          <template #header>
            <div class="space-y-2">
              ...
              <p class="max-w-2xl text-sm text-muted">
                Один проход по форме: где используется секрет, что именно сохраняем и кто за это отвечает.
              </p>
            </div>
          </template>
            ...
            <UCard variant="subtle">
              <template #header>
                <div class="space-y-1">
                  ...
                  <p class="text-sm text-muted">
                    Где используется секрет и кто отвечает за его актуальность.
                  </p>
                </div>
              </template>
              ...
              <div class="space-y-5">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                  <UFormField label="Сервис или ресурс" :error="form.errors.resource_id">
                    <USelectMenu
                  ...
                  <UFormField label="Ответственный" :error="form.errors.owner_user_id">
                    <USelectMenu
              ...
              <UCollapsible v-model:open="showAdvanced" class="mt-4 flex flex-col gap-3">
                <UButton
                  :label="showAdvanced ? 'Скрыть дополнительные настройки' : 'Показать дополнительные настройки'"
                  color="neutral"
                  variant="subtle"
                ...
                <template #content>
                  <UCard variant="outline">
                    <div class="grid gap-4 md:grid-cols-2">
                      ...
                      <UFormField label="Кто может раскрывать" :error="form.errors.reveal_policy">
                        <USelect
            ...
            <UCard variant="subtle">
              <template #header>
                <div class="space-y-1">
                  ...
                  <p class="text-sm text-muted">
                    Выбери способ хранения и сразу зафиксируй значение или внешний reference.
                  </p>
                </div>
              </template>

              <div class="space-y-6">
                <UFormField help="Если вставляешь реальное значение прямо сюда, оно будет зашифровано при сохранении. Если секрет уже живёт во внешнем vault, фиксируй только ссылку или путь.">
                  <URadioGroup
                    ...
                    :ui="{ item: 'flex-1' }"
                    :items="storageModeOptions.map(option => ({
                      label: option.title,
                      value: option.value,
                    }))"
                ...
                <UAlert
                  :color="isEncryptedMode ? 'success' : 'info'"
                  variant="soft"
                  :icon="isEncryptedMode ? 'i-lucide-lock-keyhole' : 'i-lucide-link-2'"
                  :title="isEncryptedMode ? 'Сырое значение будет зашифровано' : 'В реестр попадет только reference'"
                  :description="storageSummary"
                />
                ...
                <UCard variant="outline">
                  <div v-if="isEncryptedMode">
                    ...
                    <p class="mt-2 text-xs text-muted">
                      Это поле подходит для реального значения. После сохранения команда увидит только факт наличия секрета,
                      а не сам текст.
                    </p>
                  </div>
                  ...
                  <div v-else>
                    ...
                    <p class="mt-2 text-xs text-muted">
                      Укажите понятный reference, по которому команда быстро найдет источник истины без поиска по чатам.
                    </p>
                  </div>
                </UCard>
              </div>
          ...
          <template #footer>
            <div class="flex flex-wrap items-center justify-between gap-3">
              <p class="max-w-xl text-sm text-muted">
                Цель формы простая: чтобы владелец без лишних вопросов понял, куда вставить значение или ссылку.
              </p>
      ...
      <section v-else>
        ...
        >
          ...
          <div class="space-y-2 text-sm text-muted">
            <p>{{ secret.project_name }} / {{ secret.environment_name }}</p>
            <p>Ответственный: {{ secret.owner_user_name || 'не назначен' }}</p>
            <p>
              Хранение:
              {{
                secret.storage_mode === 'encrypted_value'
                  ? 'значение внутри Access Atlas'
                  : (secret.external_reference || secretStorageModeLabel(secret.storage_mode))
              }}
            </p>
            <p>Статус: {{ projectStatusLabel(secret.status) }}</p>
      ...
      <UModal
        ...
        :description="secretPendingReveal ? `Секрет «${secretPendingReveal.name}» имеет повышенную чувствительность. Укажи причину раскрытия.` : 'Укажи причину раскрытия секрета.'"
      >
        <template #body>
          <UForm
            :state="revealForm"
            class="space-y-4"
            @submit.prevent="secretPendingReveal && submitReveal(secretPendingReveal.id)"
          >
            <UFormField
              label="Причина"
              name="reason"
        ...
        <template #footer>
          <div class="flex justify-end gap-3">
            ...
            <UButton
              ...
              @click="secretPendingReveal && submitReveal(secretPendingReveal.id)"
            >
              Раскрыть секрет
            </UButton>
          </div>
      ...
      <UModal
        ...
        :title="revealedSecret ? `Секрет: ${revealedSecret.name}` : 'Раскрытый секрет'"
        :description="revealedSecret?.is_reference_only ? 'В этом случае показывается только внешний reference.' : 'Значение раскрыто один раз и уже зааудировано.'"
      >
        <template #body>
          <div v-if="revealedSecret" class="space-y-4">
            ...
            <UCard variant="subtle">
              <template #header>
                <div class="flex items-center justify-between gap-3">
                  <div>
                    <p class="text-xs font-medium uppercase tracking-[0.2em] text-muted">
                      {{ revealedSecret.is_reference_only ? 'Reference' : 'Значение' }}
                    </p>
                  ...
                  <UBadge variant="soft" color="neutral">
                    {{ revealedSecret.is_reference_only ? 'Внешний источник' : 'Encrypted value' }}
                  </UBadge>
                </div>
              </template>

              <UTextarea
                :model-value="revealedSecret.is_reference_only ? revealedSecret.external_reference || '' : revealedSecret.value || ''"
                :rows="4"
                readonly
                class="w-full"
              />
            ...
            <UCard v-if="!revealedSecret.is_reference_only && envLine" variant="outline">
              <template #header>
                <div class="space-y-1">
                  ...
                  <p class="text-sm text-muted">
                    Можно сразу вставить в переменные окружения.
                  </p>
                </div>
              </template>
