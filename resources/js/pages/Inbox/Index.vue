...
useFormErrorToast(
    ...
    'Не удалось добавить запись в инбокс',
);
...
Инбокс
                ...
                <p class="mt-2 max-w-2xl text-sm text-muted">
                    Сырые фрагменты чатов и ручные заметки, которые ещё нужно
                    нормализовать в структурированные сущности.
                </p>
            </section>

            <section
                v-if="props.can_manage"
                class="rounded-xl border border-default/70 bg-default p-6"
            >
                <UFormField
                    label="Предполагаемый проект"
                    :error="form.errors.suggested_project_id"
                >
                    <USelectMenu
                        ...
                        <div class="grid gap-4">
                            <div class="grid gap-4 md:grid-cols-2">
                                <UFormField label="Проект">
                                    <USelectMenu
                                        v-model="
                                            normalizeForms[item.id].project_id
                                        "
                                        value-key="value"
                                        class="w-full"
                                        :items="props.project_options"
                                ...
                                <UFormField label="Тип секрета">
                                    <UInput
                                        v-model="
                                            normalizeForms[item.id].secret_type
                                        "
                                        class="w-full"
                                        placeholder="api_key"
                                    />
                                ...
                                <UFormField label="Режим хранения">
                                    <USelect
                                        v-model="
                                            normalizeForms[item.id].storage_mode
                                        "
                                        class="w-full"
                                        :items="[
                                            {
