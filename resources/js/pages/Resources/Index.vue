...
Новый ресурс
                        ...
                        <p class="text-sm text-muted">
                            Нужен только сервис, что это за сущность, её имя и
                            кто за неё отвечает.
                        </p>
                    </div>
                </template>
                ...
                >
                    ...
                    <UFormField
                        label="ID или ссылка"
                        :error="form.errors.external_identifier"
                    >
                        <UInput
            ...
            <section v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                <UCard v-for="resource in props.resources" :key="resource.id">
                    <template #header>
                    ...
                    <template #footer>
                        <div class="flex items-center justify-between gap-3">
                            <UBadge variant="subtle" color="primary">
                                {{ resource.project_links.length }}
                                {{
                                    resource.project_links.length === 1
                                        ? 'проект'
                                        : 'проекта'
                                }}
                            </UBadge>
