...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: props.resource.name, href: route('resources.show', props.resource.id) },
]
...
useFormErrorToast(computed(() => linkForm.errors), 'Не удалось связать ресурс с проектом')

const filteredEnvironmentOptions = computed(() =>
  props.environment_options.filter(environment =>
    !linkForm.project_id || environment.project_id === linkForm.project_id,
  ),
)
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-1 flex-col gap-6 p-4">
        ...
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-3 text-sm text-muted">
              {{ props.resource.kind }}. Владелец: {{ props.resource.owner_user_name }}
            </p>
          </div>
      ...
      <UCard class="border border-default/70">
        ...
        <div v-if="props.can_manage" class="mb-4 grid gap-4 rounded-lg border border-default/60 p-4 md:grid-cols-2">
          <UFormField
            label="Проект"
            required
          ...
          <UFormField label="Окружение" :error="linkForm.errors.environment_id">
            <USelectMenu
          ...
          <UFormField label="Тип связи" :error="linkForm.errors.relation_type">
            <USelect
              ...
              :items="[
                ...
                { label: 'Поддерживающий', value: 'supporting' },
          ...
          >
            <div>
              ...
              <p class="text-xs text-muted">
                {{ projectLink.environment_name || 'Связь на уровне всего проекта' }}
              </p>
            </div>
            <div class="flex items-center gap-2">
              <UBadge variant="soft" color="primary">
