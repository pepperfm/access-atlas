...
const mainNavigation: SidebarNavItem[] = [
  { label: 'Дашборд', href: route('dashboard'), icon: 'i-lucide-layout-grid' },
  { label: 'Проекты', href: route('projects.index'), icon: 'i-lucide-folder-open-dot' },
  { label: 'Пользователи', href: route('users.index'), icon: 'i-lucide-users-round' },
  { label: 'Ресурсы', href: route('resources.index'), icon: 'i-lucide-server-cog' },
  { label: 'Доступы', href: route('access-grants.index'), icon: 'i-lucide-key-round' },
  { label: 'Секреты', href: route('secrets.index'), icon: 'i-lucide-shield-ellipsis' },
  { label: 'Ревью', href: route('reviews.index'), icon: 'i-lucide-scroll-text' },
  { label: 'Инбокс', href: route('inbox.index'), icon: 'i-lucide-inbox' },
]

const secondaryNavigation: SidebarNavItem[] = [
  { label: 'Аудит', href: route('audit.index'), icon: 'i-lucide-shield-alert' },
  { label: 'Оффбординг', href: route('offboarding.index'), icon: 'i-lucide-folder-git-2' },
]

function toNavigationGroup(id: string, label: string, items: SidebarNavItem[]): NavigationMenuItem[] {
  return [
    {
      label,
      type: 'label',
...
const userLabel = computed(() => currentUser.value.name || currentUser.value.email || 'Owner')
...
const userMenuItems = computed<DropdownMenuItem[][]>(() => [
  ...
  [
    ...
    },
    {
      ...
      onSelect: withClose(() => { colorMode.preference = 'light' }),
    ...
    },
    {
      ...
      onSelect: withClose(() => { colorMode.preference = 'dark' }),
  ...
  <UApp :toaster="{ duration: 2500 }">
    ...
    >
      <UDashboardSidebar
        ...
        :max-size="20"
        :collapsed-size="5"
        class="bg-elevated/30 sm:data-[collapsed=false]:!w-[17rem] sm:data-[collapsed=true]:!w-[5rem] max-sm:!w-[18rem]"
      >
        <template #header="{ collapsed }">
          <div class="flex items-center px-3 py-4">
            ...
            >
              <template #leading>
                <div class="flex size-8 items-center justify-center rounded-lg bg-inverted text-inverted">
